...
export async function generateFinanceTemplate(resumeData: ResumeData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      addSection(doc, "Professional Summary", "#1E3A8A", dividerColor, () => {
        writeBodyText(doc, resumeData.personalSummary?.trim() || "No personal summary provided.", textColor);
      });

      addSection(doc, "Experience", "#1E3A8A", dividerColor, () => {
      ...
      addSection(doc, "Achievements", "#1E3A8A", dividerColor, () => {
      ...
      addSection(doc, "Core Skills", "#1E3A8A", dividerColor, () => {
...
function buildHeader(doc: PDFKit.PDFDocument, resumeData: ResumeData, imageBuffer: Buffer | null, colors: { accentColor: string; mutedColor: string; dividerColor: string; }): void {
  ...
  doc.font("Helvetica").fontSize(10.5).fillColor(colors.mutedColor).text("Finance Resume", PAGE_MARGIN, titleBottomY + 4, { width: textColumnWidth, align: "left" });
...
function writeWorkExperience(doc: PDFKit.PDFDocument, exp: ResumeData["workExperiences"][number], colors: { accentColor: string; mutedColor: string; textColor: string; }): void {
  ...
  if (exp.bulletPoints?.length) {
    ...
    exp.bulletPoints.forEach((bullet) => {
      ...
      doc.font("Helvetica").fontSize(10.2).fillColor("#B45309").text("•", PAGE_MARGIN, currentY, { width: 8 });
