...
export async function generateCreativeTemplate(resumeData: ResumeData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      addSection(doc, "About Me", "#D946EF", dividerColor, () => {
        writeBodyText(doc, resumeData.personalSummary?.trim() || "No personal summary provided.", textColor);
      });

      addSection(doc, "Experience", "#D946EF", dividerColor, () => {
      ...
      addSection(doc, "Education", "#D946EF", dividerColor, () => {
      ...
      addSection(doc, "Selected Work", "#D946EF", dividerColor, () => {
      ...
      addSection(doc, "Skills & Tools", "#D946EF", dividerColor, () => {
...
function buildHeader(doc: PDFKit.PDFDocument, resumeData: ResumeData, imageBuffer: Buffer | null, colors: { accentColor: string; mutedColor: string; dividerColor: string; }): void {
  ...
  doc.font("Helvetica").fontSize(10.5).fillColor(colors.mutedColor).text("Creative Resume", PAGE_MARGIN, titleBottomY + 4, { width: textColumnWidth, align: "left" });
...
function writeWorkExperience(doc: PDFKit.PDFDocument, exp: ResumeData["workExperiences"][number], colors: { accentColor: string; mutedColor: string; textColor: string; }): void {
  ...
  if (exp.bulletPoints?.length) {
    ...
    exp.bulletPoints.forEach((bullet) => {
      ...
      doc.font("Helvetica").fontSize(10.2).fillColor("#D946EF").text("•", PAGE_MARGIN, currentY, { width: 8 });
