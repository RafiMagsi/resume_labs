...
export async function generateClassicTemplate(resumeData: ResumeData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      addSection(doc, "Professional Summary", "#374151", dividerColor, () => {
        writeBodyText(doc, resumeData.personalSummary?.trim() || "No personal summary provided.", textColor);
      });

      addSection(doc, "Work Experience", "#374151", dividerColor, () => {
      ...
      addSection(doc, "Education", "#374151", dividerColor, () => {
      ...
      addSection(doc, "Selected Highlights", "#374151", dividerColor, () => {
      ...
      addSection(doc, "Skills", "#374151", dividerColor, () => {
...
function buildHeader(doc: PDFKit.PDFDocument, resumeData: ResumeData, imageBuffer: Buffer | null, colors: { accentColor: string; mutedColor: string; dividerColor: string; }): void {
  ...
  doc.font("Helvetica").fontSize(10.5).fillColor(colors.mutedColor).text("Professional Resume", PAGE_MARGIN, titleBottomY + 4, { width: textColumnWidth, align: "left" });
...
function writeWorkExperience(doc: PDFKit.PDFDocument, exp: ResumeData["workExperiences"][number], colors: { accentColor: string; mutedColor: string; textColor: string; }): void {
  ...
  if (exp.bulletPoints?.length) {
    ...
    exp.bulletPoints.forEach((bullet) => {
      ...
      doc.font("Helvetica").fontSize(10.2).fillColor("#111111").text("•", PAGE_MARGIN, currentY, { width: 8 });
