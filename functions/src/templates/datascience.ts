...
export async function generateDataScienceTemplate(resumeData: ResumeData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      addSection(doc, "Technical Summary", "#0369A1", dividerColor, () => {
        writeBodyText(doc, resumeData.personalSummary?.trim() || "No personal summary provided.", textColor);
      });

      addSection(doc, "Professional Experience", "#0369A1", dividerColor, () => {
      ...
      addSection(doc, "Education", "#0369A1", dividerColor, () => {
      ...
      addSection(doc, "Projects & Publications", "#0369A1", dividerColor, () => {
      ...
      addSection(doc, "Tools & Skills", "#0369A1", dividerColor, () => {
...
function buildHeader(doc: PDFKit.PDFDocument, resumeData: ResumeData, imageBuffer: Buffer | null, colors: { accentColor: string; mutedColor: string; dividerColor: string; }): void {
  ...
  doc.font("Helvetica").fontSize(10.5).fillColor(colors.mutedColor).text("Data Science Resume", PAGE_MARGIN, titleBottomY + 4, { width: textColumnWidth, align: "left" });
...
function writeWorkExperience(doc: PDFKit.PDFDocument, exp: ResumeData["workExperiences"][number], colors: { accentColor: string; mutedColor: string; textColor: string; }): void {
  ...
  if (exp.bulletPoints?.length) {
    ...
    exp.bulletPoints.forEach((bullet) => {
      ...
      doc.font("Helvetica").fontSize(10.2).fillColor("#0EA5E9").text("•", PAGE_MARGIN, currentY, { width: 8 });
