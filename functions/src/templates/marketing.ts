...
export async function generateMarketingTemplate(resumeData: ResumeData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      addSection(doc, "Brand Summary", "#C026D3", dividerColor, () => {
        writeBodyText(doc, resumeData.personalSummary?.trim() || "No personal summary provided.", textColor);
      });

      addSection(doc, "Campaign Experience", "#C026D3", dividerColor, () => {
      ...
      addSection(doc, "Education", "#C026D3", dividerColor, () => {
      ...
      addSection(doc, "Projects & Awards", "#C026D3", dividerColor, () => {
      ...
      addSection(doc, "Skills", "#C026D3", dividerColor, () => {
...
function buildHeader(doc: PDFKit.PDFDocument, resumeData: ResumeData, imageBuffer: Buffer | null, colors: { accentColor: string; mutedColor: string; dividerColor: string; }): void {
  ...
  doc.font("Helvetica").fontSize(10.5).fillColor(colors.mutedColor).text("Marketing Resume", PAGE_MARGIN, titleBottomY + 4, { width: textColumnWidth, align: "left" });
...
function writeWorkExperience(doc: PDFKit.PDFDocument, exp: ResumeData["workExperiences"][number], colors: { accentColor: string; mutedColor: string; textColor: string; }): void {
  ...
  if (exp.bulletPoints?.length) {
    ...
    exp.bulletPoints.forEach((bullet) => {
      ...
      doc.font("Helvetica").fontSize(10.2).fillColor("#C026D3").text("•", PAGE_MARGIN, currentY, { width: 8 });
