...
const SIDEBAR_BG = "#3B0764";
const SIDEBAR_TEXT = "#FFFFFF";
const SIDEBAR_MUTED = "#E9D5FF";
...
export async function generateMarketingTemplate(resumeData: ResumeData): Promise<Buffer> {
  ...
  buildMainHeader(doc, "Marketing Resume", mainFrame);
...
function buildSidebar(
  ...
  if (resumeData.photoUrl) {
    ...
    renderPhoto(doc, imageBuffer, photoX, y, PHOTO_SIZE, "circle", "#FFFFFF", "#2E1065", SIDEBAR_MUTED);
    y += PHOTO_SIZE + 18;
  }
...
function buildMainHeader(doc: PDFKit.PDFDocument, subtitle: string, frame: Frame): void {
  ...
  doc
    ...
    .fontSize(24)
    .fillColor("#111111")
    .text("Marketing", frame.x, titleY, { width: frame.width, align: "left" });
...
function buildMainContent(doc: PDFKit.PDFDocument, resumeData: ResumeData, frame: Frame): void {
  ...
  renderSectionHeading(doc, "Brand Summary", headingStyle, frame);
  ...
  const highlights = [...extras.projects, ...extras.achievements].slice(0, 10);
  if (highlights.length) {
    renderSectionHeading(doc, "Campaign Highlights", headingStyle, frame);
