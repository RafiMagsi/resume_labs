...
export async function generateMarketingTemplate(resumeData: ResumeData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      addSection(doc, "Professional Summary", accentColor, dividerColor, () => {
        const summary = resumeData.personalSummary?.trim() || "Marketing professional focused on growth.";
        writeBodyText(doc, summary, textColor);
      });

      addSection(doc, "Marketing Experience", accentColor, dividerColor, () => {
        ...
        resumeData.workExperiences.forEach((exp, index) => {
          writeMarketingExperience(doc, exp, {
            accentColor,
            darkColor,
            mutedColor,
            textColor,
      ...
      addSection(doc, "Marketing Skills & Tools", accentColor, dividerColor, () => {
...
function buildHeader(
  ...
  doc
    ...
    .text(resumeData.title?.trim() || "Marketing Professional", PAGE_MARGIN, titleY, {
  ...
  doc
    ...
    .text("Strategy • Digital • Brand", PAGE_MARGIN, titleBottomY + 4, {
      width: textColumnWidth,
      align: "left",
    });
...
function drawPhotoPlaceholder(
  ...
  doc
    ...
    .fillAndStroke("#F3E8FF", borderColor)
    .restore();
...
function writeMarketingExperience(
  doc: PDFKit.PDFDocument,
  ...
  const titleLine = exp.role?.trim() || "Marketing Role";
